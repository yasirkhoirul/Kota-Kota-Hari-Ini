...
class UploadPageDartCubit extends Cubit<UploadPageDartState> {
  ...
  final PostKota kota;
  UploadPageDartCubit(this.imagePicker, {required this.kota})
    : super(UploadPageDartInitial());
  ...
  Future<void> uploadForm(String id) async {
    if (state is UploadPageDartLoaded) {
      final currentState =
          state as UploadPageDartLoaded; // gambar sudah ada disini
      try {
        if (currentState.imageBytes != null) {
          final data = await kota.execute(
            currentState.imageBytes!,
            id,
          ); // contoh upload ke usecase
